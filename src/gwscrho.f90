...
! adnj edit - read the self energy instead of calculating it (DMFT)
...
! determine the GW Fermi energy
  if(task.ne.808)  call gwefermi
  if((mp_mpi).and.(task.ne.808)) then
    write(60,'("GW Fermi energy        : ",G18.10)') efermi
    flush(60)
  end if
! determine the density and magnetisation
  call gwrhomag
! adnj edit - generate KS V_s and B_s for DMFT
  if(task.eq.808) then
...
! compute the energy components
    call energy
! adnj edit - calculate the Kohn-Sham energy eigenvectors, eigenvalues
...
! check for convergence
  if ((iscl.ge.2).or.(task.eq.808)) then !adnj edit - for DMFT+DFT
    if (mp_mpi) then
      write(60,*)
      ...
      write(65,'(G18.10)') dv
      flush(65)
      !adnj edit - put charge convergence to elk.out
      if(task.eq.808) then
        write(*,*)
        write(*,'("RMS change in Kohn-Sham potential (target) : ",G18.10," (",&
         &G18.10,")")') dv,epspot
      endif
      !end edit
    end if
...
if(task.eq.808) deallocate(dmatkdmft) !adnj edit
deallocate(vmt,vir)
