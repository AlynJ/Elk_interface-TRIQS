...
!-------------------------------------!
  do ilo=1,nlorb(is)
    l=lorbl(ilo,is)
    do io=1,apword(l,is)
      fr(1:nr)=apwfr(1:nr,1,io,l,ias)*lofr(1:nr,1,ilo,ias)*rlmt(1:nr,2,is)
      oalo(io,ilo,ias)=splint(nr,rlmt(:,1,is),fr)
    end do
  end do
!-----------------------------------------------!
  ...
  do ilo=1,nlorb(is)
    ...
    do jlo=1,nlorb(is)
      if (lorbl(jlo,is).eq.l) then
        fr(1:nr)=lofr(1:nr,1,ilo,ias)*lofr(1:nr,1,jlo,ias)*rlmt(1:nr,2,is)
        ololo(ilo,jlo,ias)=splint(nr,rlmt(:,1,is),fr)
      end if
    end do
  end do
end do
