...
if (spinsprl) then
  ...
  spinorb=.false.
  select case(task)
  case(51,52,53,61,62,63,700,701)
    write(*,*)
...
! adnj - also now ELFs, Wannier projectors and DMFT require 
! second-variational eigenvectors
select case(task)
case(5,10,51,52,53,170,300,460,461,600,620,630,640,805,806,808,809)
  tevecsv=.true.
end select
...
if (reducebf.lt.1.d0-1.d-4) then
  select case(task)
  case(0:9,28,200,201,350,351,360,630,808)! include DMFT fcsc
    continue
  case default
    bfieldc(:)=0.d0
    bfcmt(:,:,:)=0.d0
