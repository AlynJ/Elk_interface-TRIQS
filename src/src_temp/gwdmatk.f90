...
complex(8), allocatable :: d(:,:),a(:,:),work(:)
! external functions
complex(8) gtwsum
external gtwsum
...
! invert this matrix
  ...
  if (info.ne.0) then
    write(*,*)
    write(*,'("Error(gwdmatk): unable to solve the Dyson equation")')
    write(*,'(" for fermionic frequency ",I6)') iw
    write(*,*)
    stop
  end if
...
! adnj edit - jump here to diagonalise the dmft dmat
! diagonalise the density matrix for the natural orbitals and occupation numbers
!10 call eveqnz(nstsv,nstsv,d,occsv(:,ik))
10 continue 
