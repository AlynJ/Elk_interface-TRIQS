...
complex(8), allocatable :: olptemp(:,:,:)
complex(8), allocatable :: z(:),z1(:,:),z2(:,:)
real(8), allocatable :: u(:)
integer ist,ispn,lm,lmm,is,nr,ikp,iorb
integer ngp(nspnfv),l,lmmax,ia,ias
real(8) t1

!k index for reading in eigenvectors in genwfsvpwan
ikp=0
if((task.eq.811).or.(task.eq.807)) ikp=ik
...
call genwfsvpwan(.true.,.false.,ikp,nst,idx(1:nst,ik),ngdc,igfc,vkl(:,ik),ngp,igpig,wfmt,ngtc, &
       wfir)
if((task.eq.807).or.(task.eq.811)) then 
!set occupations to occmax - charges not needed for A(k,w)
  occsv(:,:) = occmax
else
...
! rotate temporary projectors into different basis
    if (lmirep) then
      ...
      z2(:,:)=sublm(1:lmmax,1:lmmax,iorb,ia)
      do ispn=1,nspinor
        do ist=1,nst
