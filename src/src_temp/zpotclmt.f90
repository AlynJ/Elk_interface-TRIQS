...
! !ROUTINE: zpotclmt
! !INTERFACE:
subroutine zpotclmt(nr,nri,ld,rl,zrhomt,zvclmt)
! !USES:
use modmain
...
real(8), intent(in) :: rl(ld,-lmaxo-1:lmaxo+2)
...
do l=0,lmaxi
  ...
  do m=-l,l
    ...
    call splintp(nr,rl(:,1),f1,f5)
    call splintp(nr,rl(:,1),f2,f1)
    call splintp(nr,rl(:,1),f3,f2)
    call splintp(nr,rl(:,1),f4,f3)
    t1=f2(nr); t2=f3(nr)
    i=lm
    do ir=1,nri
...
do l=lmaxi+1,lmaxo
  ...
  do m=-l,l
    ...
    call splintp(nro,rl(iro,1),f1(iro),f5(iro))
    call splintp(nro,rl(iro,1),f2(iro),f1(iro))
    call splintp(nro,rl(iro,1),f3(iro),f2(iro))
    call splintp(nro,rl(iro,1),f4(iro),f3(iro))
    t1=f2(nr); t2=f3(nr)
    i=npi+lm
    do ir=iro,nr
      r1=t0*rl(ir,l3); r2=t0*rl(ir,l)
