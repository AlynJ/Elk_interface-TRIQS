...
complex(8), allocatable :: zw(:),a(:,:),work(:)
...
allocate(ipiv(nm),a(nm,nm),work(nm))
vfxcp=0.d0
it=0
10 continue
...
if (mp_mpi.and.tq0) then
  do iw=1,nwrf
    eps0(1:3,1:3,iw)=epsi(1:3,1:3,iw)
    call zgetrf(3,3,eps0(:,:,iw),nm,ipiv,info)
    if (info.eq.0) call zgetri(3,eps0(:,:,iw),nm,ipiv,work,nm,info)
    if (info.ne.0) goto 20
  end do
...
! output the Faraday angle components Delta delta and Delta beta
  do iw=2,nwrf
    zw(iw)=0.5d0*zi*eps0(1,2,iw)/sqrt(eps0(1,1,iw))
  end do
...
! output the Kerr angle
  do iw=2,nwrf
    zw(iw)=-eps0(1,2,iw)/(sqrt(eps0(1,1,iw))*(eps0(1,1,iw)-1.d0))
  end do
...
! output magnetic linear dichroism (MLD) spectrum
  ...
  do iw=2,nwrf
    z1=eps0(1,1,iw)
    zw(iw)=t2*eps0(1,2,iw)/((z1-1.d0)*(z1-(t1*(z1+1.d0))))
  end do
