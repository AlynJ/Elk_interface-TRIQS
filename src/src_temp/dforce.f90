...
do ias=1,natmtot
  is=idxis(ias)
  z1=z1+zfmtinp(nrmt(is),nrmti(is),rlmt(:,3,is),zvclmt(:,ias),drhomt(:,ias))
end do
dyn(ipph,iasph)=-z1
...
do ias=1,natmtot
  is=idxis(ias)
  z1=z1+zfmtinp(nrmt(is),nrmti(is),rlmt(:,3,is),zvclmt(:,ias), &
   grhomt(:,ias,ipph))
end do
...
do ias=1,natmtot
  ...
  np=npmt(is)
  do i=1,3
    z1=zfmtinp(nr,nri,rlmt(:,3,is),grhomt(:,ias,i),dvsmt(:,ias))
...
! compute the gradient of the density derivative
  ...
  do i=1,3
    z1=zfmtinp(nr,nri,rlmt(:,3,is),zfmt,gzfmt(:,i))
    dyn(i,ias)=dyn(i,ias)-z1
  end do
end do
