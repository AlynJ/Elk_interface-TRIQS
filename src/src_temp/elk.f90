...
! check if task can be run with MPI
  if (lp_mpi.gt.0) then
    select case(task)
    case(0,1,2,3,5,15,16,28,29,61,62,63,110,120,135,136,162,170,180,185,188, &
     200,201,205,240,241,300,320,330,331,350,351,360,372,373,440,460,461,600, &
     610,620,630,640,700,701,808) !adnj edit
      continue
    case default
...
! generate the density matrix from the DMFT density matrix
  case(809)
    call gwdmat
! output the band characters for PDOS calculations
  case(810)
    call writebandchar
! output the band projectors
  case(811)
    call writewan
! end edit
  case default
...
!BOI
! !TITLE: {\huge{\sc The Elk Code Manual}}\\ \Large{\sc Version 6.2.8}\\ \vskip 20pt \includegraphics[height=1cm]{elk_silhouette.pdf}
...
!   Bartolom\'{e} and Ren\'{e} Wirnata. Special mention of David Singh's very
...
!   support of Karl-Franzens-Universit\"{a}t Graz, the EU Marie-Curie Research
!   Training Networks initiative, the Max Born Institute and the Max Planck
!   Society.
!
!   \vspace{24pt}
!   Kay Dewhurst\newline
...
!
!   \vspace{12pt}
!   Berlin, Halle and Uppsala, July 2019
!   \newpage
!
!   \section{Units}
