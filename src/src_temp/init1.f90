...
! adnj edit - include bandstr for projectors
if ((task.eq.20).or.(task.eq.21).or.(task.eq.811)) then
...
! adnj - with wannier projector routine
  if ((task.eq.100).or.(task.eq.101).or.(task.eq.102).or.(task.eq.807)) then
    ngridk(:)=np3d(:)
...
if ((task.eq.700).or.(task.eq.701)) then
...
if ((xctype(1).lt.0).or.(task.eq.5).or.(task.eq.10).or.(task.eq.205).or. &
 (task.eq.300).or.(task.eq.600).or.(task.eq.620).or.(task.eq.630).or.tddos) then
  nppt=nkptnr
else
  nppt=nkpt
end if
